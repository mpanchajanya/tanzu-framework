...
import (
	...
	config2 "github.com/vmware-tanzu/tanzu-framework/cli/runtime/config"
	"gopkg.in/yaml.v3"
)
	...
	tests := []struct {
	...
	}{
		{
			...
			opts: func(config *Config) {
				...
				config.Keys =
					[]Key{
						{Name: config2.KeyClientOptions, Type: yaml.MappingNode},
						{Name: config2.KeyCLI, Type: yaml.MappingNode},
						{Name: config2.KeyDiscoverySources, Type: yaml.SequenceNode},
					}
			},
		},
		{
